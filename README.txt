...
Step 1: Get and Build Repo
    - gets triggered by a push to a Github repository (https://github.com/anatesan-stream/jenkins-build-test-repo.git)
    - Run "mvn clean install -DskipTests" to build the repo (Skip running tests at this point, since some tests will fail by design)

Step 2: Set up the QA Environment
    - Sets up the QA environment by simulating a deployment in a QA-<Timestamp> directory
...
Step 3: Set up the Stage Environment
    - Sets up the STAGE environment by simulating a deployment in a STAGE-<Timestamp> directory
